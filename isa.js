	...
	if (isa.length === 3){
		...
		for (index = tag_split.length-1; index > -1;  index--){
			...
			if(tag_split[index] === 'NN' || tag_split[index] === 'NNS'){
				isa_split[index] = isa_split[index].replace(/(\.|;|:)/,''); //se una parola è seguita da uno dei seguenti simboli senza uno spazio la puliamo per evitare che thesaurus poi si incazzi
